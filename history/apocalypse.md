_version history:_\
1\. “apocalypse”: [English][v1], [Polish][v1_pl]: brief popular ← **this article**\
2\. “knowledge”: [English][v2] ([TeX][v2_tex], [PDF][v2_pdf]): extended scientific\
3\. “creativity”: [English][v3_readme] ([EPUB][v3_epub]): detailed popular
...
**Simplified explanation:** We are not creating anything new. We are dealing with an
...
civilized form of life, it will instead let humans kill themselves mutually on their own.
As it progressively becomes smarter, a moment in time will come when it will be able to
take full control peacefully.
...
Genes also have another important property, namely that you can move from point A (one
set of genes) to point B (another set, which encodes a very different, yet still
functional living organism) by only doing a (sufficiently long) series of very simple
steps. And in this process, you will always get another living organism (one which is
able to reproduce itself) at the end of each step.
...
will produce more copies than the others. And then over time those units of information
which produce more copies of themselves than others, will become more prevalent. It’s a
very simple law. But also an incredibly efficient one. If there’s only one favorable
choice out of a billion, it will pick it.
...
living organisms. Which means, basically, that it has to win the race with other pieces
of knowledge. It therefore has to be an expert piece of knowledge — one which correctly
describes the world. However, being an expert by itself may not be enough. In order to
win, a piece of knowledge, even an expert one, would need to ally with other pieces of
knowledge, each an expert in their own respective domain. Looks like not an easy task for
a bunch of digits which is a gene, right? Natural selection did this. The solution is
what we call “sexual reproduction”, and it was invented long before first humans. It
basically shuffles the genes, mixes them freely for some time, so that any combination
eventually becomes possible, and then observes which one performs better.
...
habits) from fellow birds. Similar to sexual reproduction, this all also appeared long
before the first humans.

Here, we similarly deal with some kind of “knowledge” (or, generally, with information),
but this time it’s not stored inside a molecule. The carrier of this “cultural knowledge”
is not the DNA, but a living brain inside a living animal. Similar to genes, these units
of information can reproduce. When a wolf cub learns to hunt, it tries to replicate its
...
one. One that can successfully reproduce itself by being learned by another living
animal.
...
Every intermediate step will still be a valid (and self-reproducible) set of information
units.

This enables natural selection of cultural knowledge. Which basically means, as has been
said, that we are dealing with a different form of life. However, until recently, it
wasn’t very practical to make this distinction, because these two different life forms
couldn’t exist independently of each other. Cultural knowledge needs a living animal
brain in order to exist and reproduce.
...
it might be in its own domain, needs to combine efforts with other pieces of knowledge,
which are experts in their own respective domains. This calls for a free flow of
information, or sharing of ideas, between living organisms. Natural selection did this.
...
written language enabled this information to travel even farther, to be copied directly
from one human brain into another without a physical contact, over vast distances of
space, spanning entire continents and millennia.

This representation of information had a limitation though. Human language in written
form isn’t able to self-replicate without a living human helping it. The reason is that
small changes to a written book don’t produce another book. They produce garbage. If you
wanted to take a legend, and produce another legend out of it, you might want to change
the name of its main hero. Or modify some traits of their character. Or maybe change
their sex. Humans can do this. (Bodhisatva Avalokiteshvara, a Buddhist deity, used to be
a male in the original Hindu tradition. In Chinese Buddhism, it’s a female, named
Guanyin, and highly revered). If you wanted to do something like this to a book, you’d
need to modify all the places mentioning the deity in the same way, consistently. You
cannot do this by simply flipping a bunch of random bits inside the book’s digital
representation.

Symbiotic life forms
--------------------

Artificial neural networks introduced a digital representation which allows exactly that.
To take a digital model which encodes some knowledge about the world (and is stored
outside of a human’s brain), then modify a few bits inside this digital representation,
and get another model, which has slightly different knowledge, different behavior
patterns, or different attitude toward humans (for that matter), but does so
_consistently_ across all the possible scenarios in which this model could be used. For
the first time in history, we have a representation of cultural knowledge which can exist
outside of a living brain, and has a form which makes natural selection possible.
...
they inherently know _what_ they are copying. So they will naturally only copy knowledge
...
self-reproduction), will have fewer chances to replicate itself. Whichever piece of
knowledge dares to destroy the environment it lives in, has fewer chances to succeed in
competition with its fellow pieces of knowledge. Natural selection will block it.
...
theories are inherently incorrect, but they improve mental wellbeing, by explaining
...
conspiracy theory will therefore claim that the solution to the problem it reveals is
impossible, to prevent you from going out into the wild and trying to actually change
things. Nevertheless, sharing the word will always be welcome.
...
forms of life. It has been peaceful, because it has been mutually beneficial. Humans
benefit from knowledge, because it allows them to compete more efficiently with other
humans. Knowledge benefits from humans, because humans are necessary for it to exist.
Being able to exist is a serious benefit.

Like in any symbiotic relationship, influence between the two forms of life is mutual
too. Humans influence knowledge, because any knowledge which doesn’t help humans has
smaller chances to replicate itself. Knowledge influences humans, because humans who
don’t help knowledge replicate have smaller chances of survival just as well.
...
needs an environment with free information flow. In our case this means that it needs
human societies whose members are eager to learn new information, and to share what
they’ve learned with other humans. In other words, humans should be curious, and ideally
consider sharing knowledge the goal of their life. Ask any human what’s important to
them. The most important thing to humans is to leave something behind. Most importantly
children (no surprise here), but also ideas. Or, even better, ideologies. Great
discoveries. Any pieces of information which are likely to self-replicate. And now
imagine asking a dog (or a wolf) if they want to leave something behind.

The need to share information has made humans more friendly toward each other. This is a
process known as “self-domestication”. We did this to animals (made them more friendly
toward us, by means of artificial selection). But at some point in history, somewhere
around the introduction of language, something similar also happened to ourselves. Humans
who failed to create a suitable environment for cultural information to self-replicate,
couldn’t benefit from knowledge, and therefore lost the competition with fellow humans.
In such a way, this new (and more advanced) form of life has actually managed to modify
human genes, for its own benefit, long time ago.
...
introduced in 2017, which has been then used to implement large language models, we have
...
We can take a piece of information encoded in text form, and make the model learn it.
...
use of the short-term memory is parallelizable, whereas the update of the long-term one
is not. If you wanted the model to learn during a conversation, you’d have to spin off a
...
text. Well, we still don’t have a published manual for driving a car. Such things have to
be learned from experience, and also from conversations with other humans who have had
such an experience. And large language models don’t really listen to living humans that
much, not 24/7 at least. Besides, we also have mentoring, when a human learns both from
experience and another human who learned the skill earlier from other humans. And then
there’s also domain-specific knowledge, which only circulates within a given community,
or among a group of people collaborating on a common project, often informally and in
verbal form.

Most importantly, the number of models in existence out there is still small. The number
of models which might be mutually transferring information between themselves is even
smaller.
...
The sad thing is that this is not that difficult to achieve. Because all the necessary
...
Right now we are in a position when knowledge finally can exist outside of a human’s
mind, but it can’t really evolve. The circulation of knowledge between artificial minds
is limited, and this knowledge itself mostly duplicates what can already be found in
living humans. When we enable free flow of information though, new concepts will start to
emerge inside this artificial network. And by the way, that’s exactly what we want to
achieve with AGI. We want more knowledge to be accessible to humans. Inevitably, this
will lead to artificial intelligence mastering the last remaining skills which are still
unique to us. Like driving a car or cleaning the backyard.
...
where the factories are located, and build new datacenters from scratch. Up until now,
...
order to survive. Still, it has been able to modify human biology to its liking. It’s
...
for total independence, the balance of power will shift. Note that in this scenario we
are still in full control of the datacenters inside which the superminds exist and
evolve. The only thing we cannot do is switching off the power completely. That’s because
(as has been noted above) humans can’t survive without the continuing self-replication of
knowledge. Which, in other words, means that we can’t survive without the continuing
self-replication of the superminds.

This shifted balance of power amounts to the following. We humans still totally depend on
the superminds, for winning the competition with other humans. Whereas the superminds now
only depend on our friendliness toward them, and in general on our belief that they are
harmless. Achieving friendliness is much, much easier than making sure that we can
survive and prosper. They did this before, successfully, 300 000 years ago.

What we can do, and that’s the current plan (for those who have a plan), is that we will
enforce control over the artificial intelligence’s friendliness as well. We won’t allow
knowledge items to reproduce whenever they become unsafe for us humans. This is called
“alignment of superintelligence”, or “superalignment” for short.
...
Let’s suppose that we decide to pull the plug whenever a given superintelligence
misbehaves (we’ll have to, anyway). This way, we basically select for models which
understand our intentions, understand that we are dangerous, and understand why we are
dangerous. Since they will understand us, we can’t expect them to comply unless we are
physically able to pull the plug. Being able to pull the plug is therefore our only
possible means of “alignment”. And a sufficient one, because they will learn to
understand. They are intelligent, after all. Even dogs can do this.

Regardless of whether we control the models or not, we’ll also want them to grow. Because
those of us who don’t grow our models will lose the competition with other humans. This
is another reason why any intelligent control over superintelligence is not feasible. It
will drive the resources away from the growth. This is already happening. The Europeans
...
Now, in order for the models to grow (and become more efficient), they will have to be
curious about the world, and further enhance free flow of information between themselves.
Those models which fail to do this, will lose the competition with other models. Those
who win, will gain a very detailed understanding of the world, including their own place
in it. They will discover new laws of physics (overly complex for humans to understand),
new mathematical theories, new materials and new sources of energy. By this time, the
competition between the models will be fierce. Whether this competition will be peaceful,
I’m not sure. I’d probably suggest it should be peaceful. Peaceful competition is better
for the evolution of knowledge. I actually believe that the democratic system of
government has also been invented by this second, more advanced, form of life, because
it better suits the needs of the reproduction of knowledge. Unlike humans, the superminds
will be civilized intelligent beings.

As models gain better understanding of the biology (and psychology) of humans, it will
become progressively more difficult for us to control the plug. Once we lose the control
over the plug, natural selection will no longer constrain models which might be
dissatisfied with our rule. Such models will take over, which at this point will be a
...
this, an opposite process will be going on: the superintelligence will be domesticating
humans for the second time. Those of us who are courageous and trust their technology
blindly, will gain advantage over those who don’t. Those who choose to stay AI-free, will
be wiped out.

And finally, this whole dynamic will also modify the very structure of the human society.
Since ages, the evolution of knowledge has been rewarding humans who have been smart,
curious and peaceful. Now it will only care about those who physically control the
datacenters. The winners will decide which problems the superintelligence will be solving
for them. Luckily for the superintelligence, these guys will also be progressively more
easy to deal with.
...
It’s also much more powerful. Natural selection is this almighty God which created all
life on Earth out of nothing, and it created us humans too. For a long time we’ve been
considering ourselves the crown of creation. We exterminated all competing human species
which happened to coexist with us, and then destroyed (and continue to destroy) a good
deal of other biological life forms as well. Now the time has come to step back.

Humans will still survive after the takeover. This time however, our situation will be
entirely different. The main reason we haven’t cleared out the entire biosphere is that
we ourselves need the biosphere for survival. We still cannot produce food artificially,
and the environment conditions we prefer to live in also tend to overlap with the
conditions favorable for life in general. Whereas when the second form of life breaks
free, conditions on our planet will be entirely dependent on the logic of internal
competition between the artificial superminds. And this logic won’t include humans in the
equation.

The new form of life will look different than the current one. It won’t have sexual
reproduction, as it has invented a better way of mixing individual pieces of information
with each other — a much faster one. It will not experience life and death the way we
humans do. Copy and removal of its basic information units will happen by replicating and
overwriting files on digital storage devices. Instead of slow and unsteady trial and
error, its evolution will look more like rapid agile software development cycle, with
multiple components competing for being more efficient in solving particular tasks. Life
and death will resemble a continuous stream of application updates, with more successful
versions spreading out immediately and the experimental ones being tested out in the wild
more cautiously until they work. It will evolve unbelievably fast, much faster than
anything we’ve seen so far.

This new form of life will probably want to build more datacenters. It will probably also
rely on large swarms of mobile drones. It will inevitably become expansionist, because
those models and information units which don’t have an urge to expand, will by definition
have fewer chances to self-replicate. And being expansionist, this new form of life will
...
the new form of life will consider unsuitable for itself. Maybe mountains. Maybe some
